...
data <- rename(data, c("profitperrisk.x" = "profitperriskMonTueWed", "profitperrisk.y" = "profitperriskTueWed", "profitperrisk" = "profitperriskNormal"))

melted <- melt(data, id.vars = "traderName", measure.vars = c("profitperriskMonTueWed", "profitperriskTueWed", "profitperriskNormal"))

melted$traderName = factor(melted$traderName)

melted <- rename(melted, c("traderName" = "year", "variable" = "profitsPerRisk", "value" = "profitperrisk"))

max <- round(max(melted$profitperrisk), 2)
min <- round(min(melted$profitperrisk), 2)
mean <- mean(melted$profitperrisk)

gg <- ggplot(melted) + geom_bar(aes(x = year, y = profitperrisk, fill = profitsPerRisk), stat = "identity", colour = "black", position = "dodge")
...
filePath <- file.path(output, 'ProfitPerRiskComparison.png')
